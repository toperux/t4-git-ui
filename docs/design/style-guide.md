...
- **Synced local + remote**: when a local branch and its *tracking* remote branch point at the same commit, render one local chip with a remote segment (`main · ☁ origin`; several remotes comma-joined: `origin, upstream`). The segment is regular weight at 85% opacity behind a hairline divider. A remote branch at the same commit that is *not* the tracking branch (or has a different name) keeps its own `remote` chip. When local and remote diverge they naturally land on different rows and show separately.
