...
const idFile = `${process.env.TMPDIR ?? "/tmp"}/t4-git-ui-wd-session`;
...
const wd = async (method, path, body) => {
  ...
  if (value?.error) {
    console.log(`${value.error}: ${value.message}`);
    process.exit(1);
  }
  return value;
};
const s = (path = "") => `/session/${readFileSync(idFile, "utf8")}${path}`;
...
} else if (cmd === "rclick" || cmd === "dblclick") {
  // Right-click (context menus) or two clicks in a row (double-click handlers), on the element's centre.
  const el = await wd("POST", s("/element"), { using: "css selector", value: arg });
  const button = cmd === "rclick" ? 2 : 0;
  const press = [{ type: "pointerDown", button }, { type: "pointerUp", button }];
  const actions = [{ type: "pointerMove", origin: el, x: 0, y: 0 }, ...press, ...(cmd === "dblclick" ? press : [])];
  await wd("POST", s("/actions"), {
    actions: [{ type: "pointer", id: "mouse", parameters: { pointerType: "mouse" }, actions }],
  });
  console.log(cmd === "rclick" ? "right-clicked" : "double-clicked", arg);
} else if (cmd === "key") {
