...
- **The main ruleset's bypass list — check it.** The ruleset requires one approving review, and
  `gh pr merge` on Dependabot's #8 refused until the user approved it. Yet #6 (the cargo group)
  merged minutes earlier with no review at all. Either the rule was added between the two merges
  or an admin merge bypassed it silently; the ruleset's bypass list says which. Until it is read,
  do not assume the rule protects `main`.
