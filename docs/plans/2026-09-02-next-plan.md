...
- **Smoke steps no CDP walk can reach** (this machine, by hand): most walked on 2026-09-06 (by hand:
  ...
  "Git not found", I's tag push; Resolve in editor ×2 over CDP with VSCodium). Left: **four**
  unticked lines across the two smoke docs, not the one this used to claim — a DPI change
  (`smoke-test.md:281`), plus group AC's three update boxes (`smoke-test-post-v1.md:743` a failed
  check, `:746` a failed install, `:749` deb / rpm), which landed later with in-app updates and need
  a pullable network and a Linux package.
...
- **One freshness rule, not four** — open, from the 2026-09-11 review of those fixes.
