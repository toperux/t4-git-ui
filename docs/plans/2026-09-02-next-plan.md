...
- **Smoke steps no CDP walk can reach** (this machine, by hand): the native folder pickers
  (`Ctrl+O` non-repo, `Ctrl+N` empty / existing, Choose folder…, I's "no remotes → checkbox
  absent"), **Resolve in editor**, an OS theme switch while running, the first painted frame, a DPI
  change, the "Git not found" screen, clone Cancel timing, and the two large-repo performance
  checks. The list lives in `2026-09-05-full-rewalk.md` › Not walked; tick them there.
