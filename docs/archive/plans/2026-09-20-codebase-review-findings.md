...
|---|---|---|---|---|---|---|
| P0-1 | F1 | high | CONFIRMED (libgit2 half PLAUSIBLE) | git-core stage | File paths reach libgit2 and git as glob pathspecs: discarding `pages/[id].tsx` also restores `pages/i.tsx` | done `5c014d6` — `disable_pathspec_match(true)` on `discard_paths`; `unstage_paths` rewritten by hand (`reset_default` has no literal switch); `--literal-pathspecs` on `recreate_conflict_args`, `checkout_side_args`, `history_args` and `submodule_update` |
| P0-2 | F2 | high | REPRODUCED | git-core patch | A partial line selection next to `\ No newline at end of file` glues two lines together in the index | done `0619b89` — decision D1: a selection that would leave a `+` line following a demoted no-newline line is refused (`GitError::Refused`), both directions |
| P0-3 | F3 | med | CONFIRMED | git-core diff/patch | Hunk staging in a non-UTF-8 file writes U+FFFD bytes into the index | done `4d090a8` — decision D2: `FileDiff::lossy` (`#[serde(skip)]`) set on a lossy decode; `build_patch` refuses with a UTF-8 reason. Buttons stay enabled and the refusal is a toast — deferred, `open-items.md` §I |
| P0-4 | F9 | med | CONFIRMED | tauri stage | Hunk / line indices are resolved against a diff rebuilt at click time; nothing checks it is the diff the user saw | done `9722116` — decision D3: `hunk_print` (FNV-1a over header + lines) computed on both sides of the IPC, `patch::check_seen` refuses when a touched hunk's print disagrees; shared test vectors `6b34fb19` / `53a53432` pin the frontend and backend together |
| P1-1 | F4 | med | CONFIRMED | dialogs | Push sends the bare local name: `dev` tracking `origin/develop` lands on `origin/dev`, and the toast says `→ origin/develop` | done `6a806bd` — decision D5: refspec is `branch:remoteName` when the upstream's name on the picked remote differs, the bare name otherwise; toast names what was written. Always sending `refs/heads/…` (which would also remove the tag-name ambiguity) was rejected — see `open-items.md` §I, closed will-not-fix |
| P1-2 | F6 | med | CONFIRMED | tauri ops + stores | `op://event` is broadcast: another window's failed op opens this window's dock, and two CloneDialogs cross their `opId`s | done `7d698cf` — `OpOwner` (repo's holder, or the asking window for a clone); `run_git_op` emits with `emit_to(label)`, falling back to a plain `emit` with no holder; `onOpEvent`/`onOpEventReady` listen targeted at this window |
| P1-3 | F8 | med | CONFIRMED | cli runner | The run loop ends on pipe EOF, not on git's exit: a hook that backgrounds a child holds the op lock for as long as the child lives | done `cdba0d3` — the loop waits for `child.wait()`, then gives the pumps `DRAIN_GRACE` (500 ms of silence) up to `DRAIN_CAP` (5 s) before stopping them |
| P1-4 | F7 | med | CONFIRMED | app setup | No single-instance guard: a second process wipes the first's merge-tool temp files and both overwrite `layout.json` / `recents.json` | done `1730ab1` — decision D4: `tauri-plugin-single-instance` 2.4.5 registered first; a second launch hands over to the running app, which opens another window on the start screen, and exits. Linux residual risk and the updater-restart interaction recorded in `open-items.md` §I |
| P2-1 | F5 | low | CONFIRMED | dialogs | Merge offers Squash together with "Always create a merge commit"; git always refuses the pair | done `824ffca` — Squash unticks and disables itself when Always-create-a-merge-commit is on |
| P2-2 | F10 | low | CONFIRMED | updater | Install never asks whether a git operation is running in any window | done `2e62a88` — `AppState::op_running`; `install_update` refuses while any op runs, checked before the download and again before the install |
