...
import { memo, useCallback, useEffect, useMemo, useReducer, useRef, useState, type CSSProperties, type MouseEvent, type ReactNode } from "react";
...
export function DiffViewer({ path: selectedPath, oldPath: listOldPath, stats: listStats, diff, loading, error, actions }: DiffViewerProps) {
  ...
  if (!selectedPath) body = <EmptyState icon={<FileDiff size={24} aria-hidden />} title="Select a file" />;
    ...
    body = (
      <DiffBody key={flat.view} view={flat.view} rows={flat.rows} maxCols={flat.maxCols} lang={lang} actions={lineActions} selected={sel.keys} onLineClick={onLineClick} />
    );
...
function DiffBody({ view, rows, maxCols, lang, actions, selected, onLineClick }: DiffBodyProps) {
...
const UnifiedRowView = memo(function UnifiedRowView({ row, top, lang, actions, selected, onLineClick }: UnifiedRowViewProps) {
  const style = { top };
  if (row.kind === "hunk") {
    return (
